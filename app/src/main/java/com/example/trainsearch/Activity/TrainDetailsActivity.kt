...
import com.example.trainsearch.API.TrainApiClient
import com.example.trainsearch.R
import com.example.trainsearch.Adapter.TrainAdapter
...
class TrainDetailsActivity : AppCompatActivity() {
    ...
    private val apiClient = TrainApiClient.getInstance()
    private val trainListAdapter = TrainAdapter(ArrayList())
    ...
    private fun parseResponse(response: String): List<Train> {
        ...
        try {
            ...
            for (i in 0 until jsonArray.length()) {
                ...
                val daysData = jsonObject.getJSONObject("data").getJSONObject("days")
                val daysOfWeek = mutableMapOf<String, Int>()

                // Extract weekdays data and populate the map
                for (dayKey in daysData.keys()) {
                    val dayValue = daysData.getInt(dayKey)
                    if (dayValue == 1) {
                        daysOfWeek[dayKey] = dayValue
                    }
                }

                val train = Train(trainNumber, name, startingStation, destination,daysOfWeek)
                trains.add(train)
            }
